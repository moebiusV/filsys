 ...
 *
 * An edition is described by a filsys_desc_t (constants + mode conversion,
...
static mode_t mode_to_posix(const filsys_edition_t *f, const filsys_inode_t *ip) {
    if (f->desc.to_posix_mode)
        return f->desc.to_posix_mode(f, ip);
    ...
    if (t == f->desc.ifdir)
        ...
        m |= S_IFLNK;
    else if (f->desc.ifsock && t == f->desc.ifsock)
        m |= S_IFSOCK;
    else
...
static int mode_is_dir(const filsys_edition_t *f, const filsys_inode_t *ip) {
    if (f->desc.is_dir)
        return f->desc.is_dir(f, ip);
    return (ip->mode & f->desc.ifmt) == f->desc.ifdir;
}
static int mode_is_device(const filsys_edition_t *f, const filsys_inode_t *ip) {
    if (f->desc.is_device)
        return f->desc.is_device(f, ip);
    uint32_t t = ip->mode & f->desc.ifmt;
    return t == f->desc.ifchr || t == f->desc.ifblk ||
           (f->desc.ifmpc && t == f->desc.ifmpc) || (f->desc.ifmpb && t == f->desc.ifmpb);
}
static uint32_t mode_to_disk(const filsys_edition_t *f, mode_t m, int type) {
    ...
    case FILSYS_FT_BLK: return base | f->desc.ifblk;
    case FILSYS_FT_LNK: return base | f->desc.iflnk;
    default:            return f->desc.ifreg ? (base | f->desc.ifreg) : base;
    }
}
...
static ssize_t file_read(filsys_t *fs, filsys_inode_t *ip, uint8_t *buf, size_t sz, off_t off) {
    return filsys_file_read(fs->fs, ip, buf, sz, off);
}
...
static uint64_t maxfile(const filsys_t *fs) {
    return fs->ops->max_file(&fs->fs->desc);
}
...
static int lock_image(filsys_t *fs, const char *path, uint64_t offset) {
    uint64_t len = (uint64_t)fs->fs->fsize * fs->fs->desc.bsize;
...
static int preflight_create(filsys_t *fs, filsys_inode_t *ddir, const char *name) {
    if (!mode_is_dir(fs->fs, ddir))
        return -ENOTDIR;
    if (!strcmp(name, ".") || !strcmp(name, ".."))
        return -EEXIST;
    uint32_t present;
    int rc = dir_lookup(fs, ddir, name, &present);
    if (rc == 0)
        return -EEXIST;
...
static int inode_create(filsys_t *fs, mode_t mode, uid_t uid, gid_t gid,
    ...
    ip->ino = *ino;
    ip->mode = mode_to_disk(fs->fs, mode, type);
...
int filsys_create_in(filsys_t *fs, uint32_t dir, const char *name, mode_t mode,
    ...
    if (rc) return rc;
    filsys_inode_t ddir;
    rc = read_inode(fs, dir, &ddir);
    if (rc) return rc;
    rc = preflight_create(fs, &ddir, name);
    if (rc) return rc;
    ...
    if (arc) {
        ...
        if (withdraw_name(fs, dir, name, nino) == 0)
...
int filsys_link_in(filsys_t *fs, uint32_t src_ino, uint32_t dir, const char *name) {
    ...
    if (mode_is_dir(fs->fs, &ip)) {
        int a = dir_ancestor(fs, src_ino, dir);
...
int filsys_rename_in(filsys_t *fs, uint32_t sdir, const char *sname,
    ...
    filsys_inode_t sdirip;
    int rc = read_inode(fs, sdir, &sdirip);
    if (rc) return rc;
    uint32_t sino;
    rc = dir_lookup(fs, &sdirip, sname, &sino);
    if (rc) return rc;
    filsys_inode_t sip;
    if (read_inode(fs, sino, &sip)) return -EIO;

    int isdir = mode_is_dir(fs->fs, &sip);

    filsys_inode_t tdirip;
    if (read_inode(fs, tdir, &tdirip)) return -EIO;
    ...
    if (isdir) {
        int a = dir_ancestor(fs, sino, tdir);
        if (a == 1) return -EINVAL;   /* would form a cycle */
        if (a < 0) return a;          /* cannot verify: fail closed */
    }
    ...
    rc = filsys_link_in(fs, sino, tdir, tname);
    if (rc) {
        if (had_target) {
    ...
    if (isdir) {
        ...
        if (sdir != tdir) {
            ...
            if (read_inode(fs, sdir, &fddir) == 0) {
                if (fddir.nlink > 1) fddir.nlink--;
                rc = write_inode(fs, sdir, &fddir);
                if (rc) return rc;
            }
            /* tdirip was read before filsys_link_in, which rewrote the target
             * directory's inode (its size grew to hold the new entry).  Re-read
             * it fresh so we bump nlink without clobbering that update. */
            if (read_inode(fs, tdir, &tdirip) == 0) {
                tdirip.nlink++;
                rc = write_inode(fs, tdir, &tdirip);
                if (rc) return rc;
            }
        }
        /* Freeing a replaced directory target drops its ".." entry (which points
         * at tdir), so tdir loses one reference.  In the same-parent case (sdir
