...
static void usage(const char *p) {
    fprintf(stderr,
            "usage: %s -v <pdp7|v1|v4|v5|v6|v7|32v> [-o offset=N[,version=N][,uid=N][,gid=N]]\n"
            "                [-r] [-f] [-d] <image> <mountpoint>\n"
            "       %s -v <pdp7|v1|v4|v5|v6|v7|32v> [-o offset=N] -c <image>   # integrity check\n",
            p, p);
}
...
int main(int argc, char *argv[]) {
    ...
    while ((c = getopt(argc, argv, "v:o:rfdc")) != -1) {
        ...
        case 'v': {
            ...
            if (v < 0) {
                fprintf(stderr, "%s: unknown Unix version \"%s\" (pdp7, v4, v5, v6, v7, 32v)\n",
                        argv[0], optarg);
                usage(argv[0]);
                return 2;
            }
