...
/* mkfs.filsys.c - create a Research Unix filesystem in a disk image.
 *
 * Usage:
 *     mkfs.filsys -v <edition> [-o block] [-P packing] [-a arch] [-B bsize]
 *                 [-g geom] [-m m] [-n n] image [blocks]
 *
 ...
 * starts at block 0.
 *
 * -v names the edition and is required.  Several editions are one on-disk
 * format: 1, 2 and 3 are byte-identical (bitmap allocator, 10-byte dirents,
 * root inode 41); 4, 5 and 6 are byte-identical (the V6 format); vax32 is 32V,
 ...
 * the interleave).  The rest (sysiii/sysvr2/sysvr4, xenix, bsd29/bsd211, the
 * V8/V9/V10 family) resolve through the shared edition table.
 *
 * Block layout (block 0 is the start of the filesystem):
 *     block 0    left alone (a boot block, if any, is prebsd's job)
 *     block 1    superblock
...
int main(int argc, char **argv)
{
    ...
    while ((c = getopt(argc, argv, "v:o:m:n:P:a:B:g:")) != -1) {
        switch (c) {
        case 'v':
        ...
        default:
            fprintf(stderr, "usage: mkfs.filsys -v <edition> [-o block] [-P packing] [-a arch] [-B bsize] [-g geom] [-m m] [-n n] image [blocks]\n"
